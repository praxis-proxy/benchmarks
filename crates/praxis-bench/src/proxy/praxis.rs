...
///
/// The `image` field is required and has no default: the caller supplies
/// the image of the proxy under test (`--image`), so any Praxis-compatible
...
pub struct PraxisConfig {
    ...
    /// Docker image tag of the proxy under test.
    pub image: String,
}

impl PraxisConfig {
    /// Create a config for the given proxy image under test.
    pub fn new(image: String) -> Self {
        Self {
            address: "127.0.0.1:18090".into(),
            config: super::config_dir().join("praxis.yaml"),
