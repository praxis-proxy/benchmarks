//! CLI argument types and subcommand definitions for `praxis-bench`.
...
/// CLI arguments for `praxis-bench`.
#[derive(Parser)]
#[command(name = "praxis-bench", about = "Run proxy benchmarks and generate reports")]
pub(crate) struct Args {
    /// Subcommand (visualize, compare). Omit to run benchmarks.
    #[command(subcommand)]
    ...
    /// Praxis (or compatible) Docker image to benchmark. Plug in any
    /// image that serves the Praxis config schema on the same ports.
    #[arg(long, default_value = "ghcr.io/praxis-proxy/praxis:latest")]
    pub image: String,
...
/// CLI arguments for `praxis-bench compare`.
#[derive(Parser)]
